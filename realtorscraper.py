...
for page in range(0,900):
    n_pages += 1
    direct= sep.join((cwd,'data'))
    direct = '_'.join((direct,str(n_pages)))
    direct = '.'.join((direct,'json'))
    real_url = 'https://www.realtor.com/realestateagents/phoenix_az/sort-activelistings'+'/pg-'+str(page)
